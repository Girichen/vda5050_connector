...
void VDA5050Connector::OrderCallback(const vda5050_msgs::Order::ConstPtr& msg) {
  ...
  try {
    ...
    auto error = CreateWarningError(
        "orderError", e.what(), {{static_cast<std::string>("orderId"), new_order.GetOrderId()}});
    AddInternalError(error);
  ...
  if (state.GetOrderId() == new_order.GetOrderId()) { 
    ...
    if (state.GetOrderUpdateId() > new_order.GetOrderUpdateId()) {
      ...
      // Discard any already received order updates.
              //5 Is received order update deprecated? No. And 6 Is order update currently on vehicle? Yes. //
      ...
      return;
              //5 Is received order update deprecated? No. And 6 Is order update currently on vehicle? No. //
    } else { 
      ...
      try {
        ...
        auto error = CreateWarningError("orderError", e.what(),
          {{static_cast<std::string>("orderId"), new_order.GetOrderId()},
            {static_cast<std::string>("orderUpdateId"),std::to_string(new_order.GetOrderUpdateId())}});
        AddInternalError(error);
        return;
      }
    ...
    if (state.HasActiveOrder(order)) {
      ...
      auto error = CreateWarningError("orderError", error_msg,
          {{static_cast<std::string>("orderId"), new_order.GetOrderId()},
            {static_cast<std::string>("orderUpdateId"),std::to_string(new_order.GetOrderUpdateId())}});
      AddInternalError(error);
      return;
    ...
    if (state.InDeviationRange(new_order.GetNodes().front())) {
      ...
      std::string error_msg = "Vehicle not inside the deviation range of the first node in the order.";
      auto error = CreateWarningError("orderError", error_msg,
          {{static_cast<std::string>("orderId"), new_order.GetOrderId()},
            {static_cast<std::string>("orderUpdateId"),std::to_string(new_order.GetOrderUpdateId())}});
      AddInternalError(error);
