...
bool State::InDeviationRange(vda5050_msgs::Node node) {
  ...
  double angleDiff = 
            std::abs(NormalizeAngle(state.agvPosition.theta - node.nodePosition.theta));
  std::cout << "[InDeviationRange] "
  << "AGV(x=" << state.agvPosition.x 
  ...
  << "Dist=" << vehicle_to_node_dist
  << " (limit=" << node.nodePosition.allowedDeviationXY << ") | "
  << "AngleDiff=" << angleDiff
  << " (limit=" << node.nodePosition.allowedDeviationTheta << ")"
  << std::endl;
  return (vehicle_to_node_dist <= node.nodePosition.allowedDeviationXY) &&
          (angleDiff <= node.nodePosition.allowedDeviationTheta);
}
